...
public class Exam implements StreamManager, ManageableListItem{
    ...
    public String getFullDetail() {
        ...
        total.append(this.nthItem).append(". ").append(this.getShortTitle());
        total.append("\n");
        ...
        if (this.paper != null && this.subtitle != null) {
            total.append("Paper: ").append(this.paper).append(", ");
            //bw.newLine();
            total.append("Subtitle: ").append(this.subtitle).append(", ");
            //bw.newLine();
        } else {
            total.append("Unit: ").append(this.unit).append(", ");
        }
...
//        }
        ...
        for (String part : parts) {
            if (part.startsWith("Subject:")) {
                ...
                String subjectPotentialID = subjectName.trim().toLowerCase().replaceAll("[^a-z0-9]+", "_");
                this.subject = registry.get(subjectPotentialID, Subject.class);
                ...
                if (keyVal.length < 2) {
                    throw new RuntimeException("Missing date/time after 'Exam Date:'");
                }

                String dateTimeStr = keyVal[1].trim();  // "2025-03-10 08:30"
                String[] tokens = dateTimeStr.split("\\s+");
                if (tokens.length != 2) {
                    throw new RuntimeException("Invalid Exam Date format: " + dateTimeStr);
                }

                this.examDate = CSSE7023.toLocalDate(tokens[0].trim(), " String can't be turned into a date");
                this.examTime = CSSE7023.toLocalTime(tokens[1].trim(), "String can't be turned into time");
            }

           // generate id was here
        }
